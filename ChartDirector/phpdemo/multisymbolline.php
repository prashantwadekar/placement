...
$legendBox = $c->addLegend($c->getWidth() / 2, $c->getHeight() - 12, false, "Arial Bold", 8);
...
# dataX.)
$c->addScatterLayer($dataX, (new ArrayMath($dataY))->selectEQZ($pointType, NoValue)->result(),
    "Point Type 0", StarShape(5), 15, 0xffff00);
...
$c->addScatterLayer($dataX, (new ArrayMath($dataY))->selectEQZ((new ArrayMath($pointType))->sub(1
    )->result(), NoValue)->result(), "Point Type 1", PolygonShape(6), 13, 0x00ff00);
...
$c->addScatterLayer($dataX, (new ArrayMath($dataY))->selectEQZ((new ArrayMath($pointType))->sub(2
    )->result(), NoValue)->result(), "Point Type 2", Cross2Shape(), 13, 0xff0000);
